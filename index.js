  ...
  **Just a reminder — I’m not a financial advisor, just your AI guide with solid gyaan.**
  ...
  //     const dbRef = ref(db, 'test/' + testKey);
  ...
  //     const readData = snapshot.exists() ? snapshot.val() : null;
  ...
  async function callClaudeAPI(message, context, systemPrompt = null, conversationHistory = []) {
    try {
      // Build messages array with conversation history
      const messages = [];
      
      // Add conversation history (last 3 message pairs = 6 messages)
      ...
      // Add knowledge base context to the current user message if available
      if (context) {
  ...
  app.post('/hubot/message', verifyFirebaseToken, async (req, res) => {
    try {
      ...
      try {
        ...
        const dbRef = ref(db, 'user_queries/' + userKey);
        const snapshot = await get(dbRef);
        const existingConversation = snapshot.exists() ? snapshot.val() : [];
        
        // Filter out old format entries and convert to Claude API format
        ...
        console.error('Error getting conversation history:', firebaseError);
      }

      const claudeResponse = await callClaudeAPI(cleanedMsg, kb, systemPrompt, conversationHistory);
      ...
      try {
        ...
        const snapshot = await get(dbRef);
        let conversation = snapshot.exists() ? snapshot.val() : [];
        ...
        let cleanConversation = conversation.filter(entry => {
          // Keep only entries with the new format (role + content)
          ...
          return false; // Remove any other malformed entries
        });
        ...
        if (cleanConversation.length > 6) {
          cleanConversation = cleanConversation.slice(-6);
        }
